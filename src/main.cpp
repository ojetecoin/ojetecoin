...
#if defined(NDEBUG)
# error "Ojetecoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Ojetecoin Signed Message:\n";
...
{
    // Ojetecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ojetecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ojetecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Ojetecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xaaaf7fc9b67057b09868bf06d4a1319b3db55158bf8a4be625f81b37f297dc25");
    }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Ojetecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OjetecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OjetecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OjetecoinMiner(CWallet *pwallet)
{
    printf("OjetecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ojetecoin-miner");
    ...
    try { loop {
        ...
        printf("Running OjetecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OjetecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OjetecoinMiner, pwallet));
}
